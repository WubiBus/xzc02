...
public class MyHorizontalScrollView extends HorizontalScrollView implements OnClickListener {
	...
	public void initDatas(HorizontalScrollViewAdapter mAdapter) {
		Log.e("debug", "initDatas(), mAdapter.count: " + mAdapter.getCount());
	...
	public void initFirstScreenChildren(int mCountOneScreen) {
		...
		for (int i = 0; i < mCountOneScreen; i++) {
			// 如果手机图库中图片数量少于mCountOneScreen，则需做边界处理
			if (i >= count) {
				break;
			}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (scrollX == 0 || (mContainer.getWidth() - scrollX - getWidth() > mChildWidth)) {
				loadPreImg();
			}
			break;
		}
