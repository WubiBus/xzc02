...
public class PostActivity extends BaseFragmentActivity {
	...
	// 标签(属于什么话题)
	private String[] arrTag = { "#真相大揭秘", "#萌翻全场", "#撞脸大赛", "#长姿势", "#万万没想到", "#时光机", "#在路上",
			"#看完想shi", "#内牛满面", "#神奇的脑洞", "#康帅博", "#pos机" };
	...
	private void initTag() {
		...
		if(PrefUtil.getStringValue(Config.PREF_SAVE_TAG)!=""){
			 newarrtag = getMergeArray(PrefUtil.getStringValue(Config.PREF_SAVE_TAG).replace("[", "").replace("]", "").split(","),arrTag);
		}else{
			newarrtag = arrTag;
		}
	...
	private void refreshSelectedTag() {
		...
		// 调整标签位置
		llSelectedTagContainer.getViewTreeObserver().addOnGlobalLayoutListener(
				new OnGlobalLayoutListener() {
					...
					public void onGlobalLayout() {
						...
						int width1 = llSelectedTagContainer.getWidth()
								- llSelectedTagContainer.getPaddingLeft();
	...
	public Bitmap diejia(Bitmap b, int position) {
		...
		// 设置滤镜
		GPUImageFilter filter = GPUImageFilterTools.createFilterForType(this,
	...
	private void showHandlerPicWindow(int index) {
		...
		if (index == 1) {
			if (currentLayoutStyle == LAYOUT_STYLE_HORIZONTAL) {// 水平
				contentView = LayoutInflater.from(this).inflate(
						R.layout.pop_window_handler_pic_left, null);
				popupWindow = new PopupWindow(contentView, PostActivity.PIC_WIDTH / 2,
						LayoutParams.WRAP_CONTENT);
			} else {
				contentView = LayoutInflater.from(this).inflate(
						R.layout.pop_window_handler_pic_left2, null);
			...
			if (currentLayoutStyle == LAYOUT_STYLE_HORIZONTAL) {// 水平
				contentView = LayoutInflater.from(this).inflate(
						R.layout.pop_window_handler_pic_right, null);
				popupWindow = new PopupWindow(contentView, PostActivity.PIC_WIDTH / 2,
						LayoutParams.WRAP_CONTENT);
			} else {
				contentView = LayoutInflater.from(this).inflate(
						R.layout.pop_window_handler_pic_right2, null);
				popupWindow = new PopupWindow(contentView, LayoutParams.WRAP_CONTENT,
						PostActivity.PIC_HEIGHT / 2);
			}
		}
		...
		if (index == 1) {// 左边
			if (currentLayoutStyle == LAYOUT_STYLE_HORIZONTAL) {// 水平
				int[] location = new int[2];
				rlImageContainer1.getLocationOnScreen(location);
				Log.e("=============", "x==" + location[0] + "y==" + location[1] + "==popupWindow="
	...
	private String getColor1() {
		...
		if (outerColor1 != 0) {
			return Color.red(outerColor1) + ";" + Color.green(outerColor1) + ";"
					+ Color.blue(outerColor1);
		} else {
	...
	private String getColor2() {
		...
		if (outerColor2 != 0) {
			return Color.red(outerColor2) + ";" + Color.green(outerColor2) + ";"
					+ Color.blue(outerColor2);
		} else {
...
//		}
		...
		try {
			...
			if (imageFile1 == null || imageFile2 == null || !imageFile1.exists()
					|| !imageFile2.exists()) {
					...
					public void run() {
						...
						runOnUiThread(new Runnable() {
							...
							public void run() {
								...
								params.addParameter("paragraphReplyId", UUID.randomUUID()
										.toString());
								...
								if (isEdit) {
									...
									params.addParameter("paragraphId", UUID.randomUUID().toString());
								...
								x.http().post(params, new CommonCallback<JSONObject>() {
									...
									public void onSuccess(JSONObject result) {
										...
										StatusResultEntity entity = JSON.parseObject(
												result.toString(), StatusResultEntity.class);
										if ("0".equals(entity.getStatus())) {
											if(lastTag!=null){
											if(lastTag.indexOf("本周最佳")!=-1){
												TT.show(PostActivity.this, "'本周最佳'为无比活动内部字符，无法作为话题使用，感谢您的理解!");
											}
											if(lastTag.indexOf("每日精选")!=-1){
												TT.show(PostActivity.this, "'每日精选'为无比活动内部字符，无法作为话题使用，感谢您的理解!");
											}}
											// 为了不保存数据
											isEdit = true;
											setResult(RESULT_OK);
	...
	private void onRotateClick(View v) {
		...
		RotateAnimation rotateAnimation = new RotateAnimation(currentDegrees,
				currentDegrees += 90f, Animation.RELATIVE_TO_SELF, 0.5f,
