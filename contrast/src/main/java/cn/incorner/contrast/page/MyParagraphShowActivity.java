...
/**
 * 我的对比度 页面，以gridview形式
 * 
 * @author guohaibing
 */
@ContentView(R.layout.activity_my_paragraph_show)
public class MyParagraphShowActivity extends BaseFragmentActivity implements OnTouchMoveListener, IRefreshingAnimationView {

	private static final String TAG = "MyParagraphShowActivity";
	...
	private void init() {
		...
		// 初始化刷新框架
		vHeader = getLayoutInflater().inflate(
		...
		getWindow().getDecorView().getViewTreeObserver()
				.addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
					...
					public void onGlobalLayout() {
						...
						crlContainer.setHeaderContainerHeight(llNavContainer
								.getHeight());
//
//						// position相关
//						gvContent.setSelectionFromTop(position + 1,
//								vHeader.getHeight());
					}
				});

		myParagraphAdapter = new ImageGridViewAdapter(list, getLayoutInflater());
...
//		List<ParagraphEntity> newList = getIntent()
...
//		}
	...
	private void refreshMyParagraphFromServer() {
		...
		x.http().post(params, new CommonCallback<JSONObject>() {
			...
			public void onSuccess(JSONObject result) {
				...
				ParagraphResultEntity entity = JSON.parseObject(
						result.toString(), ParagraphResultEntity.class);
		...
		crlContainer.getViewTreeObserver().addOnPreDrawListener(
				new OnPreDrawListener() {
					...
					public boolean onPreDraw() {
						...
						blur(bitmap, llNavContainer, llNavContainer.getTop(),
								true);
						return false;
					}
				});
	...
	protected void onResume() {
		...
		if (myParagraphAdapter.getCount() > 0 && gvContent.getChildCount() > 0) {
			int position = gvContent.getFirstVisiblePosition();
			int childCount = gvContent.getChildCount();
			...
			for (int i = 0; i < arrPos.length; ++i) {
				...
				x.http().post(params, new CommonCallback<JSONObject>() {
					...
					public void onSuccess(JSONObject result) {
						...
						final CommentResultEntity entity = JSON.parseObject(
								result.toString(), CommentResultEntity.class);
...
//		}
	...
	private void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		Intent intent = new Intent();
		intent.setClass(MyParagraphShowActivity.this, MyParagraphActivity.class);
		intent.putExtra("position", position);
		intent.putParcelableArrayListExtra("list", (ArrayList<ParagraphEntity>) list);
